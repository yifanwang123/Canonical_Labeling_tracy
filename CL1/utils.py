...
class NodeInvariant:
    ...
    def compute_invariant(self, cls):
        ...
        for cell_id in sorted(cell_structure):
            ...
            # sys.exit()
            cell_invariant = self.cell_invariant(cell)
            invariant.append(cell_invariant)
        # return tuple(sorted(invariant))
        return tuple(sorted(invariant))


    def cell_invariant(self, cell):
        cell_neighbors = []
        for vertex in cell:
            neighbors = self.graph.neighbor_dict[vertex]
            neighbor_degrees = [len(self.graph.neighbor_dict[neighbor]) for neighbor in neighbors]
            cell_neighbors.append(tuple(neighbor_degrees))
        return tuple(sorted(cell_neighbors))
    
    def helper(self, cell):
        cell_neighbors = {}
        for vertex in cell:
            neighbors = self.graph.neighbor_dict[vertex]
            neighbor_degrees = sorted([len(self.graph.neighbor_dict[neighbor]) for neighbor in neighbors])
    ...
    def node_each_cell_invariant(self, cell, cls):
        ...
        for vertex in cell:
            ...
            new_neighbor_rank[vertex] = self.helper(self.graph.neighbor_dict[vertex])
        ...
        for vertex in cell:
            ...
            neighbor_degrees = sorted([len(self.graph.neighbor_dict[neighbor]) for neighbor in new_neighbor_rank[vertex]])
            cell_neighbors[vertex] = neighbor_degrees
        cell = sorted(cell, key=lambda x: (cell_neighbors[x], another[x]))
        return cell
    ...
    # def trace_invariant(self, partition):
...
class TreeNode:
    def __init__(self, partition, sequence, parent=None):
