...
from SS_al2 import SchreierSims
...
def one_dimensional_refinement_helper(G, partition, sequence, node_invariant, ifprint=False):
    ...
    partition.code = node_invariant.quotien_graph_info(G, partition)
    partition.active = 1
    # print('Final ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    # print(partition.cell_dict)
...
def symmetry_pruning(traces, current_trace, generators):
    ...
    for gen in generators:
        # Apply the automorphism to the trace
        transformed_trace = tuple(gen[v] for v in current_trace)
...
def find_one_leaf(current_node, indi_node, node_invariant, graph, traces):
    ...
    if is_discrete(child_node.partition):
        ...
        return find_one_leaf(child_node, a[0], node_invariant, graph, traces)
...
def refine_partition(graph, partition, node_invariant, seen_codes, record_file):
    ...
    while current_level_nodes:
        ...
        for current_node in current_level_nodes:
            ...
            leaf_1 = find_one_leaf(current_node, a[0], node_invariant, graph, traces)
            leaves.append(leaf_1)
            seen_nodes.append(leaf_1)
            
            leaf_2 = find_one_leaf(current_node, a[1], node_invariant, graph, traces)
